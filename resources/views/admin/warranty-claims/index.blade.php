<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-100">Klaim Garansi</h2>
            @can('submit-claims')
    ...
    <div class="p-6 bg-white dark:bg-dark-eval-1 rounded-2xl border-2 border-emerald-100 dark:border-emerald-800">
        ...
        <form method="GET" class="mb-6">
            <div class="flex gap-3">
                ...
                <select name="product" class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-emerald-500 dark:bg-dark-eval-1 dark:text-gray-200">
                    <option value="">Semua Produk</option>
                    @foreach($products as $product)
                    <option value="{{ $product->variant }}" {{ request('product') == $product->variant ? 'selected' : '' }}>{{ $product->variant }}</option>
                    @endforeach
                </select>
        ...
        <div class="space-y-4">
                ...
                <div class="flex items-start justify-between">
                    <div class="flex-1">
                        ...
                        <p class="text-sm text-gray-600 dark:text-gray-400 mb-1">
                            <span class="font-semibold">Produk:</span> {{ $claim->product->standardPacking->variant ?? 'N/A' }}
                        </p>
                        <p class="text-sm text-gray-600 dark:text-gray-400 mb-1">
                            <span class="font-semibold">Tipe:</span> {{ ucfirst($claim->complaint_type) }}
                        </p>
                        <p class="text-sm text-gray-600 dark:text-gray-400">
                            <span class="font-semibold">Diajukan:</span> {{ $claim->submitted_at->format('d M Y H:i') }}
                        </p>
                    </div>
            ...
            @empty
            <div class="py-8 text-center text-gray-500">Tidak ada klaim garansi</div>
            @endforelse
        </div>
