<x-app-layout>
    ...
    <div class="space-y-6">
        @if(session('success'))
        <div class="p-4 bg-emerald-50 dark:bg-emerald-900/20 border-l-4 border-emerald-500 text-emerald-700 dark:text-emerald-400 rounded-lg">
            {{ session('success') }}
        </div>
        @endif
        ...
        <div class="p-6 bg-white dark:bg-dark-eval-1 rounded-2xl border-2 border-emerald-100 dark:border-emerald-800">
            <form method="GET" action="{{ route('users.index') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Name or email..." class="w-full px-4 py-2 border rounded-lg dark:bg-gray-800 dark:border-gray-700">
                </div>
                <div>
                    <label class="block text-sm font-medium mb-2">Filter by Role</label>
                    <select name="role" class="w-full px-4 py-2 border rounded-lg dark:bg-gray-800 dark:border-gray-700">
                ...
                <div class="flex items-end gap-2">
                    <button type="submit" class="px-6 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700">Filter</button>
                    <a href="{{ route('users.index') }}" class="px-6 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700">Reset</a>
                </div>
            </form>
        </div>
